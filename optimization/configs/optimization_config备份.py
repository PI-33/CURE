...
exp_name = "CURE_Qwen2.5-7B-Instruct_2gpu_04021809_step"
...
# Verbose reward diagnostics: embed full question, all code/test generations, execution
# matrices, EM (q,w,e), and raw reward vectors into temp_data/reward_diagnostics.json
# (copied to experiments/.../optimization/diagnostics/step_*.json). Large; enable only when debugging.
reward_diagnostics_include_rollouts = False
# Max problems per step to attach rollout_debug (None = no limit). Use 3–8 for sanity; 0 = none.
reward_diagnostics_max_rollout_problems = 8
# Truncate each string field to this many chars (None or 0 = no truncation).
